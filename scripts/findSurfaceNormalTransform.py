...
if __name__ == '__main__':
    rospy.init_node('FindSurfaceNormalTransform', anonymous = True)

    psi, theta, phi = [np.pi/8, 0, np.pi/7]
    xyz_in = [2, 1, 10]
    # tof_readings = [4, 5, 6] 
    surface_offset = 4
    SENSOR_RADIUS = 1
    SENSOR_CLOCKING = np.pi/2


    def compute_surface_normal(msg):
        tof_readings = [msg.r1, msg.r2, msg.r3]
        targetTransform = findSurfaceNormalTransform(psi, theta, phi, xyz_in, tof_readings, surface_offset, SENSOR_RADIUS, SENSOR_CLOCKING)
        print(targetTransform.strline('rpy/zyx'))

    rospy.Subscriber('tof_sensor_values', TOFSensorValues, compute_surface_normal)
    rospy.spin()